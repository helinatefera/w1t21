...
setup_docker() {
 ...
 while [ "$tries" -lt "$max_tries" ]; do
   if compose run --rm --no-deps -T tester "apk add --no-cache curl >/dev/null 2>&1; curl -s -o /dev/null -w '%{http_code}' http://backend:8080/api/setup/status | grep -qE '200|503'" >/dev/null 2>&1; then
     break
   fi
   tries=$((tries + 1))
   sleep 2
...
run_api_script_with_retry() {
 local script_name="$1"
 local cmd="apk add --no-cache bash curl python3 coreutils >/dev/null 2>&1; bash /workspace/API_tests/$script_name"
...
run_unit_tests() {
 ...
 if ! compose run --rm --no-deps -T tester "apk add --no-cache bash python3 >/dev/null 2>&1; cd frontend && { [ -d node_modules ] && echo 'Using existing node_modules' || npm install --silent; }"; then
 ...
 for test_file in "${UNIT_FILES[@]}"; do
   run_test "unit" "$(basename "$test_file")" compose run --rm --no-deps -T tester "apk add --no-cache bash python3 >/dev/null 2>&1; cd frontend && npm run -s test -- ../unit_tests/$(basename "$test_file")"
 done
}
